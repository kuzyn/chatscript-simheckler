...
static bool SetContains1(MEANING set,MEANING M, unsigned int depth) 
{
	...
	{
		SetFactBack(D,0); 
		if (trace & TRACE_INFER && CheckTopicTrace()) Log(STDUSERLOG," SetContains %s %s : ",D->word,D1->word);
	}
	...
	if (trace & TRACE_INFER && depth == 0 && CheckTopicTrace()) Log(STDUSERLOG," not within\r\n ");
	return false;
}
...
FunctionResult QueryTopicsOf(char* word,unsigned int store,char* kind) // find topics referred to by word
{
    ...
    if (trace & TRACE_INFER && CheckTopicTrace()) Log(STDUSERLOG,"QueryTopics: %s %d ",word,FACTSET_COUNT(store));
...
static bool ConceptPropogateTest(MEANING M,unsigned int mark,unsigned int depth)  // is this meaning ultimately in a marked set
{
	...
	while (F)
	{
		if (F->verb == Mmember && F->subject == M)
		{
			...
			if (trace & TRACE_INFER && CheckTopicTrace())  Log(STDUSERLOG," %d->%s ",depth,D->word);
...
{
	if (trace & TRACE_INFER && CheckTopicTrace()) Log(STDUSERTABLOG,"QUERY: %s ",kind);
	...
	{
		SET_FACTSET_COUNT(store,0); // we dont support other ops
		return 0; 
	}
...
#		the value to process will be marked AND may or may not get stored, depending on following flag being q or m
	...
	while (*++control && *control != ':' )
	{
		...
		switch(*control)
		{
			...
			case 'n':  // ignore all member facts involving topic as object
				if (trace & TRACE_QUERY && CheckTopicTrace()) Log(STDUSERLOG," ignore all member facts w topics as objects ");
				++control; 
				baseFlags |= NOTOPIC;
				continue;
			...
			case 'i': 
				++control;
				if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG," ignore #%c results ", *control);
		...
		if (choice) // we have something to follow
		{
			...
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace())  Log(STDUSERLOG,"\r\n FactField: %c(%d) ",saveMark-baseMark+'0',saveMark);
				choice = NULL;
				for (unsigned int j = 1; j <= FACTSET_COUNT(whichset); ++j) 
				{
					...
					if (trace & TRACE_QUERY  && CheckTopicTrace())  Log(STDUSERLOG," %s ",WriteMeaning(M));
			...
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG," raw ");
				++choice;
			}

			// for non-factset values of choice
			char buf[1000];
			if (trace & TRACE_QUERY  && CheckTopicTrace()) sprintf(buf,"%s #%c(%d)",choice,saveMark-baseMark+'0',saveMark);
			if (*control == 'q') 
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG,"\r\n Tag+Queue: %s ",buf);
	...
	if (*control && --control) while (*++control && *control != ':' )
	{
		...
		case '!': //   do NOT match this
			...
			if (*control == 's')  
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG," don't match subjects #%c ",*control);
				markns = baseMark + (*++control - '0');
			}
			if (*control == 'v')  
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG," don't match verbs #%c ",*control);
				marknv = baseMark + (*++control - '0');
			}
			if (*control == 'o')  
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG," don't match objects #%c ",*control);
				markno = baseMark + (*++control - '0');
			}
			if (*control == 'n')
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG," don't match concepts with noconceptlist marking ");
				noSystemFlag = true;
		...
		case 's': 
			...
			if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG," subject must be #%c ",*control);
			break;
		case 'v': 
			markv = baseMark + (*++control - '0'); 
			if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG," verb must be #%c ",*control);
			break;
		case 'o': 
			marko = baseMark + (*++control - '0'); 
			if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG," object must be #%c ",*control);
			break;
			
		//   dont pay attention to this value during search (opposite the baseOffset)
		case 'i': 
			++control;
			if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG," ignore results with #%c ",*control);
			ignoremark = (*control == '0') ? 0 : (baseMark + (*control - '0'));
			break;
		...
		case 'c': 
			...
			break;
		case '<':  case '>': 
			if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG," propogate on verb #%c ",*control);
			propogateVerb = baseMark + (*++control - '0'); //   label of verbs to propogate on
			break;
		case '@': //   where to put answers (default is store)
			if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG," store facts in @%c",*control);
			whichset = *++control - '0';
			break;
		case '^':
		...
		case 'm': 
			...
			if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG," object must ultimately be member of set marked #%c ",*control);
			break;
		default: 
	...
	if (*control && --control) while (*++control && *control != '|')
	{
		...
		case 'S': 
			if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG,"Riccochet on Subject #%c ",*control);
			rmarks = baseMark + (*++control - '0'); 
			break;
		case 'V': 
			if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG,"Riccochet on Verb #%c ",*control);
			rmarkv = baseMark + (*++control - '0'); 
			break;
		case 'O': 
			if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG,"Riccochet on Object #%c ",*control);
	...
	if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG,"Start processing loop\r\n");
	...
	while (scanIndex < queueIndex)
	{
		...
		while (F)
		{
			...
			WORDP S;
			char factid[MAX_WORD_SIZE];
			if (G->flags & FACTSUBJECT) 
			{
				sprintf(factid,"%d",(unsigned int)G->subject);
				S = StoreWord(factid);
			}
			else S = Meaning2Word(G->subject);
			WORDP V;
			if (G->flags & FACTVERB) 
			{
				sprintf(factid,"%d",(unsigned int)G->verb);
				V = StoreWord(factid);
			}
			else V = Meaning2Word(G->verb);
			WORDP O;
			if (G->flags & FACTOBJECT) 
			{
				sprintf(factid,"%d",(unsigned int)G->object);
				O = StoreWord(factid);
			}
			else O = Meaning2Word(G->verb);

			//   if this is part of ignore set, ignore it (not good if came via verb BUG)
			if (ignoremark && OTHER->inferMark == ignoremark ) 
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG,"ignore ");
				continue;
			}
			// pay no attention to topic facts
			if (baseFlags & NOTOPIC && O->internalBits & TOPIC)
			{
				if (trace & TRACE_QUERY  && CheckTopicTrace()) Log(STDUSERLOG,"notopic ");
				continue;
			}
