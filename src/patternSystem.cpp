	...
	while (ALWAYS) //   we have a list of things, either () or { } or [ ].  We check each item until one fails in () or one succeeds in  [ ] or { }
    {
        ...
        switch(c) 
        {
			...
			case '@': // factset ref
				...
				{
					firstMatched = 10000; // cannot retry this match
            ...
            case '>': //   sentence end marker
				...
				else // you can go to end from anywhere
				{
					matched =  true;
            ...
            case '^': //   function call, function argument  or indirect function variable assign ref like ^$$tmp = null
				...
				else if (D->x.codeIndex) // system function - execute it
                {
					...
					else if (IsComparison(*ptr) && (*ptr != '!' || ptr[1] == '='))  // ! w/o = is not a comparison
					{
						char op[10];
						char* opptr = ptr;
						*op = *opptr;
				...
				{ 
					...
					if ((trace & TRACE_PATTERN || D->internalBits & TRACE_MACRO)  && CheckTopicTrace()) Log(STDUSERLOG,"("); 
					...
					while (*ptr && *ptr != ')' ) 
					{
						...
						if ((trace & TRACE_PATTERN || D->internalBits & TRACE_MACRO)  && CheckTopicTrace()) Log(STDUSERLOG," %s, ",arg); 
					}
					if ((trace & TRACE_PATTERN || D->internalBits & TRACE_MACRO)  && CheckTopicTrace()) Log(STDUSERLOG,")\r\n"); 
					...
					oldecho = echo;
					oldtrace = trace;
					if (D->internalBits & TRACE_MACRO  && CheckTopicTrace()) 
					{
            ...
            case '(': case '[':  case '{': // nested condition (required or optional) (= consecutive  [ = choice   { = optional
                hold = wildcardIndex;
				{
					...
					if (matched) 
					{
						...
						if (positionEnd) oldEnd = (reverse) ? (positionEnd + 1) : (positionEnd - 1); //   nested checked continuity, so we allow match whatever it found - but not if never set it (match didnt have words)
        ...
        if (kind == '(') //   ALL must match in sequence
        {
			...
			if (!matched)
			{
				if (wildstart && firstMatched > 0 && firstMatched < 10000) //   we are top level and have a first matcher, we can try to shift it
				{
