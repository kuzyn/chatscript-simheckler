		...
		*rawbuffer = 0;
		...
		while ((++k + i) <= endSentence)
		{
			...
			else
			{
				...
				// dont do uppercase, that should have come from raw
			}
...
void MarkAllImpliedWords()
{
	...
	//		if (originalLower[i]) flags |= originalLower[i]->properties & (NOUN_SINGULAR|NOUN_PLURAL|NOUN_PROPER_SINGULAR|NOUN_PROPER_PLURAL);
		...
		if (flags & (ADJECTIVE_NUMBER|NOUN_NUMBER) && IsPlaceNumber(original)) 
		{
			flags |= PLACE_NUMBER;
		...
		if (!IS_NEW_WORD(CU)) StdMark(MakeTypedMeaning(CU,0, (unsigned int)(finalPosValues[i] & BASIC_POS)), i, i);
