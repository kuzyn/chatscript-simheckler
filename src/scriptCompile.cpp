...
void ScriptWarn()
{
	...
	{
		++hasWarnings; 
		if (currentFilename && *currentFilename) Log(STDUSERLOG,"*** Warning- line %d of %s: ",currentFileLine,currentFilename);
...
static void FindDeprecated(char* ptr, char* value, char* message)
{
	...
	while (at)
	{
		at = strstr(at,value);
		if (!at) break;
		if (*(at-1) != '$' && lastDeprecation != at && !IsAlphaUTF8(at[len]) && !IsDigit(at[len]) ) 
		{
			lastDeprecation = at;
			BADSCRIPT(message);
		}
...
static void ValidateCallArgs(WORDP D,char* arg1, char* arg2,char argset[50][MAX_WORD_SIZE])
{
	...
	{
		if (*arg2 && stricmp(arg1,"input") && stricmp(arg1,"output")) 
			BADSCRIPT("CALL- 63 call to ^setrejoinder requires INPUT or OUTPUT as the 1st arg.")
		if (!*arg2 && (!stricmp(arg1,"input") || !stricmp(arg1,"output"))  )
	...
	while (ALWAYS) //   read as many tokens as needed to complete the call, storing them locally
	{
		...
		default:
			...
			if (parenLevel == 1)  
			{
				...
				{
					WORDP D = FindWord(word,0,LOWERCASE_LOOKUP);
					if (*nextToken == '(' && (!D || !(D->internalBits & FUNCTION_NAME))) BADSCRIPT("CALL-1 Call to function not yet defined %s",word)
...
char* ReadOutput(char* ptr, FILE* in,char* &data,char* rejoinders,char* supplement,WORDP call)
{
	...
	*hold = 0;
	while (ALWAYS) //   read as many tokens as needed to complete the responder definition
	{
		...
		{
			bool notCall = false;
			if (*nextToken == '(' || IsDigit(word[1]))   //   function call or function var ref probably
			{
				WORDP D = FindWord(word,0,LOWERCASE_LOOKUP);
				if (*nextToken == '(' && (!D || !(D->internalBits & FUNCTION_NAME))) 
					BADSCRIPT("OUTPUT-5 Apparent call to %s is not yet defined",word)
				if (*nextToken != '(' && !IsDigit(word[1])) notCall = true;
				if (!notCall)
				{
	...
	*currentTopicName = 0;
	...
	while (ALWAYS) //   read as many tokens as needed to complete the definition
	{
		...
		default:
			...
			else if (!keywordsDone) ptr = ReadKeyword(word,ptr,notted,quoted,topic,0,false,0,false);//   absorb keyword list
...
static void WriteExtendedFacts(FILE* factout,FILE* dictout,uint64 build)
{
	...
	shared = false;
	char* ptr = WriteUserVariables(buffer,false);
	shared = oldshared;
