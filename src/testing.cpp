...
reloop:
	...
	while (*file)
	{
		...
		else if (!stricmp(word,"ambig")) // show ambiguous sentences  - ambig 3
		{
			ambig = true;
		...
		else if (IsDigit(*word)) sentenceLengthLimit = atoi(word);
...
retry:
			...
			else if (!stricmp(tags[i],"VBZ")) // 3ps
			{
				...
				else Log(STDUSERLOG,"** Bad VBZ (present_3ps) %s word %d line %d:  %s\r\n",mytags[i],i,currentFileLine,buffer);
			}
	...
	Log(STDUSERLOG,"\r\n");
	ignoreRule = -1;
	// Current Status 12-25-14 
	//RAW:  ambigwords: 15001 notwrong:91797 wrong:1198 total:92995 percent:98 sentences:3939 parsed:0 parseBad:0 badSentenceRate:0  initialAmbiguousWords:47471 percentambigLeft:31 initialAmbigpercent:50
	// 93257 will be 99%

	//FULL: tokensec: 966 right:88162 wrong: 5024 total:93186 percent:94 sentences:4129 parsed:3939 parseBad:439 badSentenceRate:11 ambigWords:897 initialAmbiguousWords:47471 percentambigLeft:30 initialAmbigpercent:50
} // 89489 will be 95%
...
{
	if (!strchr(D->word,'_')) return;
	if ((D->properties & PART_OF_SPEECH) != ADJECTIVE) return;
	if (D->systemFlags & CONDITIONAL_IDIOM) return;
	Log(STDUSERLOG,"%s ADJECTIVE ADJECTIVE_NORMAL CONDITIONAL_IDIOM  ?=R\r\n",D->word);
}
...
TestMode Command(char* input,char* output)
{
	char word[MAX_WORD_SIZE];
	bool oldecho = echo;
...
static void C_TopicDump(char* input)
{
	...
	for (unsigned int i = 1; i <= numberOfTopics; ++i) 
	{
		...
		// dump keywords
		WORDP D = FindWord(name);
		FACT* F = GetObjectHead(MakeMeaning(D));
...
static void C_PermanentVariables(char* input)
{
	char word[MAX_WORD_SIZE];
	NextInferMark();
	for (unsigned int topicid = 1; topicid <= numberOfTopics; ++topicid) 
	{
		...
		while (data && *data) // walk data
		{
			...
			*at = 0;
			...
			if (!D || D->inferMark == inferMark) continue;
			D->inferMark = inferMark;
			Log(STDUSERLOG,"%s\r\n",word);
		}
	}
...
static void C_Trace(char* input)
{
	...
	while (input) 
	{
		...
		else if (*word == '-') // remove this flag
		{
			...
			else if (!stricmp(word,"uservar")) flags &= -1 ^ TRACE_USERVAR;
			...
			else if (!stricmp(word,"deep")) flags &= -1 ^ (TRACE_USERFN|TRACE_SAMPLE|TRACE_INFER|TRACE_SUBSTITUTE|TRACE_HIERARCHY| TRACE_FACTCREATE| TRACE_VARIABLESET| TRACE_QUERY| TRACE_USER|TRACE_POS|TRACE_TCP|TRACE_USERCACHE|TRACE_SQL|TRACE_LABEL|TRACE_USERVAR); 
		}
		...
		else if (!stricmp(word,"uservar")) flags |= TRACE_USERVAR;
		...
		else if (!stricmp(word,"deep")) flags |= (TRACE_USERFN|TRACE_SAMPLE|TRACE_INFER|TRACE_SUBSTITUTE|TRACE_HIERARCHY| TRACE_FACTCREATE| TRACE_VARIABLESET| TRACE_QUERY| TRACE_USER|TRACE_POS|TRACE_TCP|TRACE_USERCACHE|TRACE_SQL|TRACE_LABEL|TRACE_USERVAR); 
		...
		else if (*word == '~') // tracing a topic or rule by label
		{
			...
			if (period) *period = 0;
			unsigned int topic = FindTopicIDByName(word);
			if (!period) SetTopicDebugMark(topic,!TI(topic)->topicDebug);
...
{//  actual command names must be lower case 
	...
	{":permanentVariables",C_PermanentVariables,"What permanent variables does your script refer to"}, 
