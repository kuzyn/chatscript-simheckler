...
static void HandleFirstWord() // Handle capitalization of starting word of sentence
{
	...
	else if (E && E->properties & AUX_VERB && (N = FindWord(wordStarts[2])) && (N->properties & (PRONOUN_BITS | NOUN_BITS) || English_GetSingularNoun(wordStarts[2],true,false))) wordStarts[1] = E->word; // potential aux before obvious noun/pronoun, use lower case of it
...
{ // n is words merged into "word"
    ...
    if (n && IsUpperCase(*word->word) && word->properties & PART_OF_SPEECH  && !IS_NEW_WORD(word)) return word;//  We will merge proper names later.  words declared ONLY as interjections wont convert in other slots
	if (!n) return 0;
