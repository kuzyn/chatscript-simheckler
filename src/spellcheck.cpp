...
void InitSpellCheck()
{
	...
	{
		if (!D->word || !IsAlphaUTF8(*D->word) || D->length >= 100  || strchr(D->word,'_')) continue; // || D->internalBits & UTF8
...
bool SpellCheckSentence()
{
	...
	for (unsigned int i = FindOOBEnd(1); i <= wordCount; ++i)
	{
		...
		if (D && (GetSubjectHead(D) || GetObjectHead(D) || GetVerbHead(D))) continue;

		uint64 expectedBase = 0;
		if (ProbableAdjective(word,len,expectedBase) && expectedBase) continue;
		expectedBase = 0;
		if (ProbableAdverb(word,len,expectedBase) && expectedBase) continue;
...
static char* StemSpell(char* word,unsigned int i)
{
    ...
    else if (!strnicmp(word+len-3,"ing",3))
    {
        ...
        if (best && FindWord(best)) return English_GetPresentParticiple(best);
	}
