...
{ // this work mostly only happens on first startup, not on a restart
	...
	*loginID = 0;
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		else if (!stricmp(argv[i],"noserverprelog")) serverPreLog = 0;
...
void FinishVolley(char* incoming,char* output,char* postvalue)
{
	...
	if (!documentMode)
	{
		...
		if (userLog && prepareMode != POS_MODE && prepareMode != PREPARE_MODE)
		{
			...
			else if (!*incoming) Log(STDUSERLOG,"Start: user:%s bot:%s ip:%s rand:%d (%s) %d ==> %s  When:%s Version:%s Build0:%s Build1:%s 0:%s F:%s P:%s ",loginID,computerID,callerIP,randIndex,activeTopic,volleyCount,Purify(output),when,version,timeStamp0,timeStamp1,timeturn0,timeturn15,timePrior); // conversation start
			else 
			{
				Log(STDUSERLOG,"Respond: user:%s bot:%s ip:%s (%s) %d  %s ==> %s  When:%s ",loginID,computerID,callerIP,activeTopic,volleyCount,incoming,Purify(output),when);  // normal volley
				if (volleyCount == 15 && timeturn15[1]) Log(STDUSERLOG," F:%s ",timeturn15);
			}
...
unsigned int ProcessInput(char* input,char* output)
{
	...
	int oldInputRejoinderTopic = inputRejoinderTopic;
	int oldInputRejoinderRule = inputRejoinderRuleID;
	...
	{
		TestMode commanded = DoCommand(buffer,output);
		if (!strnicmp(buffer,":retry",6) || !strnicmp(buffer,":redo",5))
		{
...
loopback:
	...
	while (nextInput && *nextInput && loopcount < 50) // loop on user input sentences
	{
		...
		if (result == RETRYINPUT_BIT)
		{
			inputRejoinderTopic  = oldInputRejoinderTopic; 
			inputRejoinderRuleID = oldInputRejoinderRule; 
			ResetTopicSystem();
...
void OnceCode(const char* var,char* function) //   run before doing any of his input
{
	...
	if (trace & (TRACE_MATCH|TRACE_PREPARE) && CheckTopicTrace()) 
	{
		...
		{
			Log(STDUSERLOG,"\r\nPrePass\r\n");
			if (trace & TRACE_VARIABLE) DumpUserVariables();
		}
		else 
		{
			Log(STDUSERLOG,"\r\n\r\nPostPass\r\n");
